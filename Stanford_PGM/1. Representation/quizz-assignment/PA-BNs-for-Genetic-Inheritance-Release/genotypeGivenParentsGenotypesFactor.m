...
genotypeFactor.card = (numAlleles * (numAlleles - 1) / 2 + numAlleles) * ones(1, 3);
...
for i = 1 : prod(genotypeFactor.card(2 : 3))
  g = zeros(1, genotypeFactor.card(1));
  fatherAllels = genotypesToAlleles(Assignment(i, 1), :);
  motherAllels = genotypesToAlleles(Assignment(i, 2), :);
  for j = 1 : 2
    for k = 1 : 2
      g(allelesToGenotypes(fatherAllels(j), motherAllels(k))) = g(allelesToGenotypes(fatherAllels(j), motherAllels(k))) + 1;
    end
  end
  genotypeFactor.val((i - 1) * genotypeFactor.card(1) + 1 : i * genotypeFactor.card(1)) = g / 4;
end
