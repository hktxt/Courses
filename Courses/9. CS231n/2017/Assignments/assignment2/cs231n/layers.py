...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_ = (x - sample_mean) / sample_std

        out = gamma * x_ + beta

        # update cache for backward pass
        cache = (x, x_, gamma, sample_mean, sample_var, sample_std)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x, x_, gamma, sample_mean, sample_var, sample_std = cache
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx =(1 / N) * gamma * 1/sample_std * ((N * dout) - np.sum(dout, axis=0) - (x - sample_mean) * np.square(1/sample_std) * np.sum(dout * (x - sample_mean), axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
