...
p = zeros(prod(phenotypeFactor.card(2 : end)), 1);

for i = 1 : prod(phenotypeFactor.card(2 : end))
  for j = 1 : numVar1
    p(i) = p(i) + alleleWeights{j}(assignment(i, j));
  end
  for j = 1 : numVar2
    p(i) = p(i) + alleleWeights{j}(assignment(i, j + numVar1));
  end
end

sigmoidP = computeSigmoid(p);
phenotypeFactor.val(1 : 2 : end - 1) = sigmoidP;
phenotypeFactor.val(2 : 2 : end) = 1 - sigmoidP;
